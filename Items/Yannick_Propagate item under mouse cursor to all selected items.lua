-- @description Yannick_Propagate item under mouse cursor to all selected items
-- @author Yannick
-- @version 1.0
-- @about
...
-- @donation https://www.paypal.com/paypalme/yaunick?locale.x=ru_RU
  ...
  for i=1, #t_items do
    if t_items[i][1] == sel_item then
      ...
      reaper.SetEditCurPos(t_items[i][2],0,0)
      reaper.SetOnlyTrackSelected(t_items[i][3], true)
      reaper.Main_OnCommand(42398,0) --- past items with pool
  ...
  reaper.Undo_EndBlock("Propagate item under mouse cursor to all selected items",-1)
  reaper.PreventUIRefresh(-1)
