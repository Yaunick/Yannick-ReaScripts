-- @description Yannick_Bypass all FX from selected tracks - Restore previous (slot n)
-- @author Yannick
-- @version 1.1
-- @about
...
-- @donation https://telegra.ph/How-to-send-me-a-donation-04-14
  ...
  while bool_ext == false do
    ...
    local retval, key, val = reaper.EnumProjExtState(0, "Selected_tracks_fx_yannick_reasc_bypassver" .. slot, i_ext)
          ...
          for i_fx=0, reaper.TrackFX_GetCount(t_tracks[i_sel_tracks])-1 do
            ...
            for i_fx_guid=1, #t_dual do
              if GUID == t_dual[i_fx_guid][1] then
                reaper.TrackFX_SetEnabled(t_tracks[i_sel_tracks], i_fx, t_dual[i_fx_guid][2])
  ...
  if exist_ext == false then
    if show_warning_window == true then
      reaper.MB("No saved bypass states of tracks FX! (slot " .. slot .. ")", "Error",0)
    end
    nothing() return
  end
  
  reaper.Undo_EndBlock('Bypass all FX from selected tracks - Restore previous (slot ' .. slot .. ')', -1)
  reaper.PreventUIRefresh(-1)
